...
public class MilvusVectorStoreAdapter implements VectorStore {
    ...
    public List<String> search(String query, Long agentId, int topK) {
        ...
        try {
            ...
            List<org.springframework.ai.document.Document> results = memoryStore.similaritySearch(request);
    ...
    public List<String> searchKnowledge(String query, Long agentId, int topK) {
        ...
        try {
            ...
            List<org.springframework.ai.document.Document> results = knowledgeStore.similaritySearch(request);
    ...
    public void store(Long agentId, String content, Map<String, Object> metadata) {
        ...
        try {
            ...
            org.springframework.ai.document.Document document = new org.springframework.ai.document.Document(content, enrichedMetadata);
    ...
    public void storeBatch(Long agentId, List<String> contents) {
        ...
        try {
            List<org.springframework.ai.document.Document> documents = contents.stream()
                    .map(content -> new org.springframework.ai.document.Document(content, Map.of(
                            "agent_id", agentId,
    ...
    public List<Document> similaritySearch(SearchRequest request) {
        ...
        try {
            ...
            org.springframework.ai.vectorstore.SearchRequest springAiRequest = toSpringAiSearchRequest(request);

            // 使用 knowledgeStore 执行检索
            List<org.springframework.ai.document.Document> springAiResults = knowledgeStore.similaritySearch(springAiRequest);

            // 转换为 domain 层的 Document
            return springAiResults.stream()
    ...
    public void addDocuments(List<Document> documents) {
        ...
        try {
            ...
            List<org.springframework.ai.document.Document> springAiDocuments = documents.stream()
     ...
     */
    private org.springframework.ai.document.Document toSpringAiDocument(Document domainDoc) {
     ...
     */
    private Document toDomainDocument(org.springframework.ai.document.Document springAiDoc) {
     ...
     */
    private org.springframework.ai.vectorstore.SearchRequest toSpringAiSearchRequest(SearchRequest domainRequest) {
