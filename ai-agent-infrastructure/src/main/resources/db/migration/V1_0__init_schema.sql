...
CREATE TABLE `email_log` (
  ...
  `type` varchar(20) NOT NULL COMMENT '邮件类型: VERIFY_CODE, NOTIFICATION',
  `content` text COMMENT '邮件内容',
  `status` int(11) DEFAULT 0 COMMENT '发送状态: 0-待发送, 1-成功, 2-失败',
...
CREATE TABLE `node_template` (
  ...
  `type_code` varchar(50) NOT NULL COMMENT '节点类型代码: START, END, LLM, TOOL, CONDITION, HTTP',
  `name` varchar(100) NOT NULL COMMENT '模板名称',
  `description` text COMMENT '模板描述',
  `icon` varchar(200) DEFAULT NULL COMMENT '图标标识',
  `default_schema_policy` json DEFAULT NULL COMMENT '默认Schema策略',
  `initial_schema` json DEFAULT NULL COMMENT '初始化Schema (inputSchema/outputSchema)',
...
CREATE TABLE `sys_config_field_def` (
  ...
  `field_type` varchar(20) NOT NULL COMMENT '字段类型: text, number, boolean, select, textarea, json',
...
CREATE TABLE `node_template_config_mapping` (
  ...
  `is_required` int(11) DEFAULT 0 COMMENT '是否必填: 0-否, 1-是',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `workflow_node_execution_log` (
  ...
  `render_mode` varchar(20) DEFAULT NULL COMMENT '渲染模式: TEXT, MARKDOWN, JSON',
  `status` int(11) DEFAULT 0 COMMENT '执行状态: 0-待执行, 1-执行中, 2-成功, 3-失败, 4-跳过',
...
CREATE TABLE `workflow_human_review_record` (
  ...
  `decision` varchar(20) DEFAULT NULL COMMENT '审核决定: APPROVED, REJECTED, MODIFIED',
  `trigger_phase` varchar(20) NOT NULL COMMENT '触发阶段: BEFORE_EXECUTION, AFTER_EXECUTION',
