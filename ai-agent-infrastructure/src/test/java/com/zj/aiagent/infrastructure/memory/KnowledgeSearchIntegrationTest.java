...
class KnowledgeSearchIntegrationTest {
    ...
    /** 模拟文档：关键词密度从高到低 */
    private static final String DOC_HIGH = "向量数据库是专门用于存储和检索向量数据的数据库系统，向量数据库支持高效的相似度检索";
    private static final String DOC_MED = "知识库管理系统可以使用向量数据库来实现语义搜索功能";
    private static final String DOC_LOW = "今天学习了Spring Boot微服务架构的最佳实践和部署方案";
    ...
    void setUp() {
        ...
        List<Document> docs = List.of(
                new Document("id-1", DOC_HIGH, Map.of("dataset_id", DATASET_ID, "chunk_index", 0)),
                new Document("id-2", DOC_MED, Map.of("dataset_id", DATASET_ID, "chunk_index", 1)),
                new Document("id-3", DOC_LOW, Map.of("dataset_id", DATASET_ID, "chunk_index", 2))
        );
    ...
    void semantic_search_returns_results() {
        List<String> results = adapter.searchKnowledgeByDataset(DATASET_ID, "向量数据库检索", 3);

        assertFalse(results.isEmpty(), "语义检索应返回结果");
        assertTrue(results.size() <= 3, "结果数不应超过 topK");
        // 所有结果都应来自我们存入的文档
        for (String r : results) {
            assertTrue(r.equals(DOC_HIGH) || r.equals(DOC_MED) || r.equals(DOC_LOW),
                    "结果应来自已存入的文档");
        }
    }
    ...
    void semantic_search_wrong_dataset_returns_empty() {
        List<String> results = adapter.searchKnowledgeByDataset("ds-nonexistent", "向量数据库", 3);
    ...
    void keyword_search_reranks_by_keyword_score() {
        List<String> results = adapter.keywordSearchByDataset(DATASET_ID, "向量数据库检索", 3);

        assertFalse(results.isEmpty(), "关键词检索应返回结果");
        // 高关键词密度文档应排在前面
        if (results.size() >= 2) {
            double scoreFirst = KeywordScorer.score("向量数据库检索", results.get(0));
            double scoreLast = KeywordScorer.score("向量数据库检索", results.get(results.size() - 1));
            assertTrue(scoreFirst >= scoreLast,
                    "第一条结果的关键词得分(" + scoreFirst + ")应 >= 最后一条(" + scoreLast + ")");
        }
    }
    ...
    void keyword_search_irrelevant_query() {
        List<String> results = adapter.keywordSearchByDataset(DATASET_ID, "量子纠缠超导体", 3);
    ...
    void hybrid_search_fuses_scores() {
        List<String> results = adapter.hybridSearchByDataset(DATASET_ID, "向量数据库检索", 3);
    ...
    void hybrid_differs_from_pure_strategies() {
        ...
        List<String> semantic = adapter.searchKnowledgeByDataset(DATASET_ID, query, 3);
        List<String> keyword = adapter.keywordSearchByDataset(DATASET_ID, query, 3);
        List<String> hybrid = adapter.hybridSearchByDataset(DATASET_ID, query, 3);
        ...
        assertEquals(semantic.size(), hybrid.size(), "混合检索和语义检索应返回相同数量的结果");
    }
    ...
    void topk_one_returns_single_result() {
        List<String> semantic = adapter.searchKnowledgeByDataset(DATASET_ID, "向量", 1);
        List<String> keyword = adapter.keywordSearchByDataset(DATASET_ID, "向量", 1);
        List<String> hybrid = adapter.hybridSearchByDataset(DATASET_ID, "向量", 1);
    ...
    void underscore_dataset_id_filter_works() {
        List<String> results = adapter.searchKnowledgeByDataset(DATASET_ID, "测试", 10);
        assertFalse(results.isEmpty(), "下划线 dataset_id 过滤应能匹配文档");
    }

    @Test
    @DisplayName("驼峰 datasetId 存储的文档用下划线过滤应匹配不到（暴露历史 Bug）")
    void camelCase_metadata_mismatch_exposed() {
        // 模拟 AsyncDocumentProcessor 历史行为：用驼峰 datasetId 存储
        InMemoryVectorStore buggyStore = new InMemoryVectorStore();
        MilvusVectorStoreAdapter buggyAdapter = new MilvusVectorStoreAdapter(buggyStore, memoryStore);

        buggyStore.add(List.of(
                new Document("id-buggy", "这是一条用驼峰字段名存储的文档",
                        Map.of("datasetId", "ds-buggy-001")) // 注意：驼峰
        ));

        // 用下划线 dataset_id 过滤 —— 应该匹配不到
        List<String> results = buggyAdapter.searchKnowledgeByDataset("ds-buggy-001", "文档", 5);
        assertTrue(results.isEmpty(),
                "驼峰 datasetId 存储的文档用下划线 dataset_id 过滤应匹配不到，这暴露了 AsyncDocumentProcessor 的历史 Bug");
    }
