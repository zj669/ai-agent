...
const mockStoreState = {
  ...
  markDirty: vi.fn(() => { mockStoreState.isDirty = true }),
  markClean: vi.fn(() => { mockStoreState.isDirty = false }),
  toggleNodeExpand: vi.fn(),
  setOperationState: vi.fn((s: string) => { mockStoreState.operationState = s as 'idle' | 'saving' | 'publishing' }),
...
let capturedOnConnect: ((conn: { source: string; target: string }) => void) | null = null

vi.mock('@xyflow/react', () => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function ReactFlow(props: any) {
    capturedOnConnect = props.onConnect
    const nodes = props.nodes as Array<{ id: string; data: { label: string; nodeType: string } }>
    return (
      <div data-testid="reactflow-canvas">
        {nodes?.map((n: { id: string; data: { label: string; nodeType: string } }) => (
...
const { default: WorkflowEditorPage } = await import('../pages/WorkflowEditorPage')
