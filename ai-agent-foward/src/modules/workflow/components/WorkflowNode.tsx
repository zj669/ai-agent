...
import type { ConditionConfig, Branch as ConditionBranch } from './ConditionBranchEditor'

export type WorkflowNodeType = 'START' | 'END' | 'LLM' | 'CONDITION' | 'TOOL' | 'HTTP' | 'KNOWLEDGE'
...
const NODE_STYLES: Record<WorkflowNodeType, { bg: string; border: string; icon: string; accent: string }> = {
  ...
  KNOWLEDGE: { bg: 'bg-teal-50', border: 'border-teal-300', icon: '📚', accent: 'text-teal-600' },
}

const NODE_TYPE_LABELS: Record<WorkflowNodeType, string> = {
  START: '开始', END: '结束', LLM: 'LLM', CONDITION: '条件', TOOL: '工具', HTTP: 'HTTP', KNOWLEDGE: '知识库',
}
...
function WorkflowNode({ id, data, selected }: WorkflowNodeProps) {
  ...
  const hasSchema = (nodeData.inputSchema && nodeData.inputSchema.length > 0) || (nodeData.outputSchema && nodeData.outputSchema.length > 0) || nodeData.policy
  ...
  const handleConfigChange = useCallback((key: string, value: unknown) => {
    setNodes((nds) =>
      nds.map((n) => {
        ...
        return {
          ...n,
          data: { ...d, userConfig: { ...(d.userConfig ?? {}), [key]: value } },
        }
      })
    )
  }, [id, setNodes])

  const handleInputSchemaChange = useCallback((schema: FieldSchema[]) => {
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id !== id) return n
        const d = n.data as unknown as WorkflowNodeData
  ...
  const handleOutputSchemaChange = useCallback((schema: FieldSchema[]) => {
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id !== id) return n
        const d = n.data as unknown as WorkflowNodeData
  ...
  const handleConditionConfigChange = useCallback((condConfig: ConditionConfig) => {
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id !== id) return n
        const d = n.data as unknown as WorkflowNodeData
        // Sync branches for handle rendering
        const branches: Branch[] = condConfig.branches.map((b: ConditionBranch) => ({
          id: b.id,
          name: b.name,
        }))
        return {
          ...
          data: {
            ...d,
            branches,
            userConfig: { ...(d.userConfig ?? {}), conditionConfig: condConfig },
          },
        }
      })
    )
  ...
  const conditionConfig: ConditionConfig | null = isCondition
    ? (nodeData.userConfig?.conditionConfig as ConditionConfig) ?? {
        ...
        branches: (nodeData.branches ?? []).length > 0
          ? (nodeData.branches ?? []).map((b, i, arr) => ({
              id: b.id,
              name: b.name,
              type: (i === 0 ? 'if' : i === arr.length - 1 ? 'else' : 'elseif') as 'if' | 'elseif' | 'else',
              priority: i + 1,
              logic: 'AND' as const,
              conditions: [{ sourceRef: '', operator: 'EQUALS', value: '', valueType: 'literal' as const }],
            }))
          : [
              { id: `branch-default-if`, name: '如果', type: 'if' as const, priority: 1, logic: 'AND' as const, conditions: [{ sourceRef: '', operator: 'EQUALS', value: '', valueType: 'literal' as const }] },
              { id: `branch-default-else`, name: '否则', type: 'else' as const, priority: 2, logic: 'AND' as const, conditions: [] },
            ],
      }
    : null
  ...
  useEffect(() => {
    if (isCondition && conditionConfig && (!nodeData.branches || nodeData.branches.length === 0) && !didInitBranches.current) {
      didInitBranches.current = true
      handleConditionConfigChange(conditionConfig)
    }
  }, [isCondition, conditionConfig, nodeData.branches, handleConditionConfigChange])
      ...
      {isCondition && (
        <div className="border-t border-slate-200">
          {(nodeData.branches ?? []).map((branch) => (
            <div key={branch.id} className="relative flex items-center pl-3 pr-5 py-2 border-b border-slate-100 last:border-b-0">
              <div className="flex items-center gap-1.5 flex-1 min-w-0">
                <div className={cn(
                  'h-2 w-2 rounded-full shrink-0',
                  branch.name === '否则' ? 'bg-slate-400' : 'bg-amber-500'
                )} />
                <span className="text-xs text-slate-600 truncate">{branch.name}</span>
              </div>
              <Handle
                type="source"
