...
describe("knowledge main flow", () => {
  ...
  it("上传文档成功后显示反馈并刷新文档列表", async () => {
    ...
    const file = new File(["hello knowledge"], "guide.txt", {
      type: "text/plain",
    });
    ...
    expect(
      await screen.findByText("上传成功，正在处理..."),
    ).toBeInTheDocument();
  });

  it("切换到语义分块后应携带对应参数上传", async () => {
    render(<KnowledgePage />);
    ...
    fireEvent.click(screen.getByRole("radio", { name: "语义分块" }));

    const maxChunkInput = screen.getByDisplayValue("800");
    fireEvent.change(maxChunkInput, { target: { value: "900" } });

    const file = new File(["hello knowledge"], "semantic.txt", {
      type: "text/plain",
    });
    const input = document.querySelector('input[type="file"]');
    expect(input).not.toBeNull();
