...
public class ClientLoadDataStrategy implements ILoadDataStrategy{
    @Resource
    private IAgentArmoryRepository agentArmoryRepository;
    @Resource
...
//        }, threadPoolExecutor);
        ...
        CompletableFuture<Void> futureMcp = CompletableFuture.runAsync(() -> {
            agentArmoryRepository.queryMcps(context);
        }, threadPoolExecutor);
        // advisor
//        CompletableFuture<Void> futureAdvisor = CompletableFuture.runAsync(() -> {
//            agentArmoryRepository.queryAdvisors(context);
//        }, threadPoolExecutor);
//        // prompt
...
//        }, threadPoolExecutor);
        ...
        CompletableFuture<Void> future = CompletableFuture.allOf(futureMcp);
        future.join();
        log.info("结束异步加载数据");
    }
}
