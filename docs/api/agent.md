...
- 后端 `AgentController` 当前没有 `/api/agent/offline`。
- 前端 `shared/api/adapters/agentAdapter.ts` 中存在 `offlineAgent(id)`，会调用 `POST /api/agent/offline`；这是当前前后端偏移点，不能把它写成已可用接口。
...
- 前端在画布编辑后将节点和边序列化为 `WorkflowGraphDTO`：
  - 当前核心服务：`ai-agent-foward/src/modules/workflow/api/workflowService.ts`
  - 当前编辑页：`ai-agent-foward/src/modules/workflow/pages/WorkflowEditorPage.tsx`
- 保存时调用：`POST /api/agent/update`
  - 前端请求位置：`ai-agent-foward/src/shared/api/adapters/agentAdapter.ts`
  - graphJson 序列化位置：`ai-agent-foward/src/modules/workflow/api/workflowService.ts`
- 后端接收并更新：
  - Controller：`ai-agent-interfaces/src/main/java/com/zj/aiagent/interfaces/agent/web/AgentController.java`
  - 请求字段：`ai-agent-application/src/main/java/com/zj/aiagent/application/agent/dto/AgentRequest.java`
  - 应用服务写入：`ai-agent-application/src/main/java/com/zj/aiagent/application/agent/service/AgentApplicationService.java`
...
- LLM 节点优先读取 `llmConfigId`，并兼容旧的 `model/baseUrl/apiKey` 或 `llm_model/llm_base_url/llm_api_key`：`ai-agent-infrastructure/src/main/java/com/zj/aiagent/infrastructure/workflow/executor/LlmNodeExecutorStrategy.java`
...
实现备注：
- `publishedVersionId` 当前保存的是发布版本号，不是 `agent_version` 表主键 ID。
- `update` 会更新草稿图和乐观锁版本，不会自动把状态切为 DRAFT；状态由 publish/rollback/delete 等显式动作改变。
