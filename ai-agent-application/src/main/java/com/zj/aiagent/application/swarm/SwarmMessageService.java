...
public class SwarmMessageService {
    ...
    public SwarmMessageDTO sendMessage(
        Long groupId,
        SendMessageRequest request
    ) {
        SwarmGroup group = groupRepository
            .findById(groupId)
            .orElseThrow(() ->
                new IllegalArgumentException("Group not found: " + groupId)
            );
        String senderRole = agentRepository
            .findById(request.getSenderId())
            .map(agent -> agent.getRole() != null ? agent.getRole() : "unknown")
            .orElse("unknown");

        log.info(
            "[Swarm] Message accepted: workspace={}, group={}, sender={}, senderRole={}, contentType={}, preview={}",
        ...
        log.info(
            "[Swarm] Message persisted: workspace={}, group={}, sender={}, senderRole={}, messageId={}, contentType={}",
            group.getWorkspaceId(),
            groupId,
            request.getSenderId(),
            senderRole,
        ...
        eventPublisher.publishEvent(
            new com.zj.aiagent.application.swarm.event.SwarmMessageSentEvent(
                this,
                groupId,
                request.getSenderId(),
                group.getWorkspaceId()
        ...
        log.info(
            "[Swarm] Message event published: workspace={}, group={}, sender={}, messageId={}",
            group.getWorkspaceId(),
            groupId,
            request.getSenderId(),
            message.getId()
     ...
     * 拉取群消息（可选标记已读）
     */
    public List<SwarmMessageDTO> getMessages(
        Long groupId,
        boolean markRead,
        Long readerId
    ) {
    ...
    public List<SwarmGroupDTO> listGroups(Long workspaceId, Long agentId) {
        ...
        return groups
            ...
            .map(group -> {
                ...
                List<SwarmMessage> messages = messageRepository.findByGroupId(
                    group.getId()
                );
                ...
                if (!messages.isEmpty()) {
                    ...
                    if (agentId != null) {
                        Long lastRead = groupRepository.getLastReadMessageId(
                            group.getId(),
                            agentId
                        );
                        unreadCount = (int) messages
                            .stream()
                            .filter(m -> m.getId() > lastRead)
                            .count();
                    }
                }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public SwarmGroupDTO createP2PGroup(
        Long workspaceId,
        Long agentId1,
        Long agentId2
    ) {
        // 先检查是否已存在
        List<SwarmGroup> existingGroups = groupRepository.findByWorkspaceId(
            workspaceId
        );
        for (SwarmGroup g : existingGroups) {
            ...
            if (
                memberIds.size() == 2 &&
                memberIds.contains(agentId1) &&
                memberIds.contains(agentId2)
            ) {
                return SwarmGroupDTO.builder()
                    .id(g.getId())
