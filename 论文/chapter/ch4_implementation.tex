...
用户进入系统后首先访问认证页面。页面提供登录、注册和找回密码三个入口，
其中注册与找回密码均需要先填写邮箱并点击“发送验证码”按钮。
前端在验证码发送成功后展示倒计时，倒计时结束前禁止重复发送，
同时对邮箱格式、密码长度和确认密码一致性进行表单校验。

当用户提交登录表单后，前端将邮箱和密码发送至认证接口。
若认证成功，系统将返回访问令牌并保存到前端状态中，随后跳转到平台首页；
...
代码4-3展示了 Agent 发布版本快照的核心逻辑，发布时基于当前草稿构造版本快照并更新已发布指针：

\begin{lstlisting}[language=Java, caption={代码4-3 AgentApplicationService -- Agent 发布关键代码}]
...
Agent 编排页面面向应用构建者，是系统中最核心的配置界面。
...
如果存在必填参数缺失或连线不完整，页面会以错误提示的方式引导用户继续修改。
...
业务使用者在前端选择已发布的 Agent 后，可以填写运行输入并点击“开始执行”按钮。
...
工作流执行过程中，前端会将每个节点的运行日志集中展示在日志面板中。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{p{2.6cm}p{5.2cm}X}
    \toprule
    \textbf{维度} & \textbf{BEFORE\_\allowbreak EXECUTION} & \textbf{AFTER\_\allowbreak EXECUTION} \\
    \midrule
    触发时机 & 节点执行前 & 节点执行后 \\
    节点是否已执行 & 否 & 是 \\
...
恢复后的界面会将原先暂停的节点从橙色等待状态切换为运行中或已完成状态，
并在日志面板中追加审批通过记录，展示审批人、审批时间和修改内容摘要。

如果人工检查点配置在节点执行前，恢复后当前节点会重新进入执行队列；
如果配置在节点执行后，恢复后系统会直接推进后继节点。
用户可以在界面上观察到后续节点依次变为运行中和已完成状态，
从而确认人工审核没有中断整体执行链路，而是作为可控闸门嵌入工作流执行过程。
审批通过后工作流恢复执行界面如图~\ref{fig:resume_ui}~所示。
...
代码4-11展示了文档上传和异步处理入口的关键逻辑：

\begin{lstlisting}[language=Java, caption={代码4-11 KnowledgeApplicationService -- 文档上传与异步处理}]
@Transactional
...
知识库页面面向应用构建者，主要用于维护 Agent 执行所需的外部知识。
...
代码4-12展示了异步文档处理与批量向量化的关键实现：

\begin{lstlisting}[language=Java, caption={代码4-12 AsyncDocumentProcessor -- 文档分块与向量化处理}]
@Async
...
代码4-13展示了基于向量数据库的检索增强接口：

\begin{lstlisting}[language=Java, caption={代码4-13 MilvusVectorStoreAdapter -- 知识检索接口}]
...
在文档完成分块和向量化后，用户可以通过知识库页面的检索入口输入测试问题，
...
\subsubsection{（2）解决方案}

系统在 \texttt{start()} 调用时，在初始化节点状态之前优先执行 \textbf{Kahn 拓扑排序}：
从入度为 0 的节点出发，逐步消除出边并将入度降为 0 的后继节点加入队列，
直到所有节点处理完毕。若最终处理节点数小于图中总节点数，说明存在环路，
立即抛出 \texttt{IllegalStateException}，并在接口层返回 HTTP 400。

\begin{lstlisting}[language=Java, caption={代码4-14 WorkflowGraph.validateAcyclic() -- 拓扑排序环检测}]
...
审批通过后调度器会\textbf{重新执行}该节点（因为节点执行前被拦截，尚未运行）。
如果不加额外控制，节点重新执行时 \texttt{checkPause()} 会再次判定满足条件，
导致工作流陷入"无限暂停"的死循环。

\subsubsection{（2）解决方案}

系统在 \texttt{Execution} 聚合根中维护 \texttt{reviewedNodes} 集合，
记录所有已完成审批的节点 ID。\texttt{checkPause()} 的首要判断为：

\begin{lstlisting}[language=Java, caption={代码4-15 checkPause() -- 已审批节点跳过逻辑}]
...
系统实现了\textbf{服务端主动心跳机制}：
在 \texttt{SseEmitter} 建立后，后台定时任务每 \textbf{15 秒}推送一次
...
系统将文档处理链路设计为\textbf{异步解耦}模式：
上传接口仅完成文件合法性校验、MinIO 存储和 \texttt{PENDING} 状态记录后\textbf{立即返回}，
再通过 Spring \texttt{@Async} 触发\textbf{异步处理器}在独立线程中完成后续处理，
并在处理完成后回写文档状态为 \texttt{COMPLETED} 或 \texttt{FAILED}。

前端通过轮询文档列表接口观察状态变化，实现非阻塞的文档接入体验。
该设计使上传接口响应时间从同步场景的数十秒降至 \textbf{500ms 以内}（
