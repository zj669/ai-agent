...
| `src/modules/mcp/` | MCP server 配置与工具查看模块 |
| `src/modules/settings/` | 设置模块 |
| `src/shared/api/` | **API 适配层**：httpClient、errorMapper、共享 adapter（agent/auth/chat/knowledge/metadata/review/dashboard） |
...
当前并非所有业务 API 都放在 `shared/api/adapters`。例如：

- Chat 流式执行：`src/modules/chat/api/chatService.ts`
- Workflow 保存/发布：`src/modules/workflow/api/workflowService.ts`
- LLM Config：`src/modules/llm-config/api/llmConfigService.ts`
- MCP：`src/modules/mcp/api/mcpAdapter.ts`
- Swarm/Writing：`src/modules/swarm/api/swarmService.ts`
...
### Chat 发送链路
- 当前聊天页发送消息的主路径是 `modules/chat/api/chatService.ts` 直接 `POST /api/workflow/execution/start` 并读取 SSE 流。
- `ChatController.sendMessage` 后端接口仍存在，但不是当前前端主路径。
- Workflow start 请求会带 `inputs.inputMessage`，后端 `SchedulerService.extractUserQuery(...)` 会按 `inputMessage -> input -> query -> message` 顺序提取用户输入。
